...
class Invaders(GameApp):
    ...
    def start(self):
        ...
        if (self._state == STATE_INACTIVE):  
            ...
            self._text.font_size=60
        else:
            self._text = GLabel(text=None)
    ...
    def _determineState(self):
        ...
        This method checks for a key press, and if there is one, it changes
        the state.  A key press is when a key is pressed for the FIRST TIME.
        In order to change the state, the user must release the key and
        press it again to change the state.
        """
        curr_keys = self.input.key_count
        if (curr_keys > 0 and self.lastkeys == 0):

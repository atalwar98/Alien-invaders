...
class Ship(GImage):
    ...
    def __init__(self, x=GAME_WIDTH/2, y=SHIP_BOTTOM + SHIP_HEIGHT/2, width=SHIP_WIDTH, height=SHIP_HEIGHT, source='ship.png'):
        """
        Initializer: Creates a ship
        """
...
class Alien(GImage):
    ...
    def __init__(self, x=0, y=0, width=ALIEN_WIDTH, height=ALIEN_HEIGHT, source=ALIEN_IMAGES[0]):
        """
        Initializer: Creates an alien
        """
        super().__init__(x = x, y = y, width = width, height = height, source = source)
...
class Bolt(GRectangle):
    ...
    def __init__(self, x=GAME_WIDTH/2, y=SHIP_BOTTOM + SHIP_HEIGHT +
                 BOLT_HEIGHT/2, width=BOLT_WIDTH, height=BOLT_HEIGHT,
                 fillcolor = "green", linecolor = "black", velocity=BOLT_SPEED):
        """
        Initializer: Creates a bolt and sets it velocity
        """
    ...
    def _isPlayerBolt(self):
        ...
        Distinguishes between bolts shot from player versus those shot from an
        alien. All bolts that travel up are fired by the player.
        """
        if self._velocity > 0:
            return True
        else:
